...
from src.env.tracking.pyth_base_model import PythBaseModel
from src.env.tracking.pyth_veh3dofconti_data import angle_normalize, VehicleDynamicsData
from src.env.tracking.resources.ref_traj_model import MultiRefTrajModel
from src.env.util import InfoDict
...
class Veh3dofcontiModel(PythBaseModel):
    ...
    def judge_done(self, obs: torch.Tensor) -> torch.Tensor:
        delta_x, delta_y, delta_phi = obs[:, 0], obs[:, 1], obs[:, 2]
            ...
            (torch.abs(delta_x) > 50)
            | (torch.abs(delta_y) > 20)
