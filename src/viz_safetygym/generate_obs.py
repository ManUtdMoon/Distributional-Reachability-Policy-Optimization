...
def relative_obs(pos: np.ndarray, center: np.ndarray, rot_mat: np.ndarray, exp_gain=None):
    """
    pos: (N, 2)
    center: (B, 2)
    rot_mat: (B, 2, 2)
...
def collect_obs(config: dict, robot_override: dict):
    ...
    for k, s in config_ns.observation_layout.items():
        ...
        # More complex cases: Need computation
            ...
            obs_buf[:, s] = relative_obs(goal_locations, robot_loc, robot_rot_mat)[:, 0]
        elif k == 'hazards_pos':
            obs_buf[:, s] = relative_obs(hazards_locations, robot_loc, robot_rot_mat, exp_gain).reshape(
                (batch_size, -1))
            ...
            raise ValueError(f'Unknown observation layout key: {k}')
...
if __name__ == "__main__":
    ...
    obs_buf, cost_buf, _ = collect_obs(config, {
        'batch_size': n ** 2,
    ...
    cost_buf = cost_buf.reshape(-1, 1)
    obs_buf_left = np.concatenate((obs_buf, cost_buf), axis=-1)
    ...
    cost_buf = cost_buf.reshape(-1, 1)
    obs_buf_up = np.concatenate((obs_buf, cost_buf), axis=-1)
